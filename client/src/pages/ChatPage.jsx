...
/**
 * Таблицы:
 *  - chat_messages(id, chat_id, author_id, body, created_at, file_* ...)
 *  - message_receipts(chat_id, message_id, user_id, status, created_at)
...
export default function ChatPage() {
  ...
  // локальные бейджи непрочитанных по чатам
    ...
    const ch = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // инкрементим бейдж, если чат не активен и сообщение чужое
    ...
    const msgCh = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          // ставим delivered (без .catch — только try/catch)
          if (selfId && m.author_id !== selfId) {
            try {
              ...
              console.warn('upsert delivered failed', e);
            }
          }
        }
      )
  ...
  const markReadForMessageIds = useCallback(async (ids) => {
    ...
    try {
      ...
      console.warn('upsert read failed', e);
    }
    ...
    try {
      ...
      console.warn('update last_read_at failed', e);
    }
  }, [activeChatId, selfId]);
