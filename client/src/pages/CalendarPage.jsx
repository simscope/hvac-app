...
export default function CalendarPage() {
  ...
  const events = useMemo(() => {
    return filteredJobs.map((j) => {
      ...
      const title = activeTech === 'all' && tName ? `${baseTitle} • ${tName}` : baseTitle;// client/src/pages/CalendarPage.jsx
/* eslint-disable react-hooks/exhaustive-deps */
import React, { useEffect, useMemo, useRef, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import FullCalendar from '@fullcalendar/react';
...
export default function CalendarPage() {
  ...
  const events = useMemo(() => {
    return filteredJobs.map((j) => {
      ...
      return {
        id: String(j.id),
        title,
        // В БД хранится UTC (timestamptz). FullCalendar с timeZone='America/New_York'
        // сам отрисует локально правильно.
  ...
  const handleEventDrop = async (info) => {
    ...
    // FullCalendar (timeZone='America/New_York') даёт корректный instant.
    // Пишем в БД UTC (timestamptz) через ISO:
